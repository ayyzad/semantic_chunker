#uses a exponential moving average
import os
import numpy as np
import logging
...
def cosine_similarity_next_five(embeddings):
    ...
    for i in range(num_embeddings - 2):
        ...
        logging.info(f"Sentence {i}: Cosine similarities for next five sentences: {current_similarities}")
...
def trailing_average_similarity(embeddings, window_size, weight_current, std_factor, weight_func_type, alpha):
    ...
    for i in range(len(similarities)):
        ...
        logging.info(f"Sentence {i}: Trailing average: {trailing_avg}, Std Dev: {trailing_std}, Weighted Avg: {weighted_avg}")
    ...
    smoothed_trailing_averages = exponential_moving_average(trailing_averages, alpha)
...
def save_chunks_to_json(chunks, output_file='chunked_transcript_EPA.json'):
...
def plot_similarities_and_trailing_averages(sentence_indices, flattened_similarities, trailing_averages, chunk_points):
    ...
    # Set x-ticks to show every sentence index
    plt.xticks(sentence_indices, rotation=90)
    plt.tight_layout()
    plt.show()
